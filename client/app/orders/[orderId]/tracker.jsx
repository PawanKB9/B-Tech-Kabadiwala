...
export default function OrderTracker({
  ...
  const defaultSteps = [
    ...
    { key: "Out for Pickup", title: "Out For Pickup" },
    { key: "Arrived", title: "Arrived" },
    { key: "Sold", title: "Sold" },
  ];

  // Normalize status mapping
  let finalStatus = status === "Picked" ? "Sold" : status;
  ...
  const enrichedSteps = steps.map((step) => {
    ...
    return {
      ...
      description: historyItem?.note || step.description || "",
    };
  });

  const activeIndex = Math.max(
    0,
    enrichedSteps.findIndex((s) => s.key === finalStatus)
  );

  const lastIndex = enrichedSteps.length - 1;

  const progressPercent =
    lastIndex > 0
      ? (Math.min(activeIndex, lastIndex) / lastIndex) * 100
      : 0;
